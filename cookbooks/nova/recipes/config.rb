...
if glance_servers and (glance_servers.length > 0)
...
if node[:nova][:network_type] == "flat"
  ...
  fixed_interface = Chef::Recipe::Barclamp::Inventory.get_network_by_type(network, "nova_fixed").interface
  node[:nova][:dhcp_vlan_network][:flat_interface] = fixed_interface
